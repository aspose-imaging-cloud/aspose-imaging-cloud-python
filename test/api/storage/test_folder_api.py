...
class TestFolderApi(StorageApiTester):
    ...
    def test_create_folder(self):
        ...
        try:
            if self.imaging_api.object_exists(requests.ObjectExistsRequest(folder, self.test_storage)).exists:
                self.imaging_api.delete_folder(requests.DeleteFolderRequest(folder, self.test_storage, True))

            self.assertFalse(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(folder, self.test_storage)).exists)

            self.imaging_api.create_folder(requests.CreateFolderRequest(folder, self.test_storage))
            self.assertTrue(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(folder, self.test_storage)).exists)
    ...
    def test_copy_folder(self):
        ...
        try:
            ...
            self.imaging_api.copy_folder(
                requests.CopyFolderRequest(self.original_data_folder, folder, self.test_storage,
                                           self.test_storage))
            self.assertTrue(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(folder, self.test_storage)).exists)

            original_files = self.imaging_api.get_files_list(requests.GetFilesListRequest(self.original_data_folder,
    ...
    def test_move_folder(self):
        ...
        try:
            ...
            if self.imaging_api.object_exists(requests.ObjectExistsRequest(tmp_folder, self.test_storage)).exists:
                self.imaging_api.delete_tmp_folder(requests.DeleteFolderRequest(tmp_folder, self.test_storage, True))

            self.assertFalse(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(tmp_folder, self.test_storage)).exists)

            self.imaging_api.copy_folder(
                requests.CopyFolderRequest(self.original_data_folder, tmp_folder, self.test_storage,
                                           self.test_storage))
            self.assertTrue(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(tmp_folder, self.test_storage)).exists)

            self.imaging_api.move_folder(requests.MoveFolderRequest(tmp_folder, folder, self.test_storage,
                                                                    self.test_storage))
            self.assertFalse(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(tmp_folder, self.test_storage)).exists)
            self.assertTrue(
    ...
    def test_files_list(self):
        ...
        files = self.imaging_api.get_files_list(
            requests.GetFilesListRequest(self.original_data_folder + '/' + folder1.name, self.test_storage)).value
        ...
        files = self.imaging_api.get_files_list(
            requests.GetFilesListRequest(self.original_data_folder + '/' + folder2.name, self.test_storage)).value
        ...
        files = self.imaging_api.get_files_list(
            requests.GetFilesListRequest(self.original_data_folder + '/' + folder2.name + '/' + folder3.name,
                                         self.test_storage)).value
        self.assertEqual(1, len(files))
