...
class TestFileApi(StorageApiTester):
    ...
    def test_copy_download_file(self):
        ...
        try:
            if self.imaging_api.object_exists(requests.ObjectExistsRequest(folder, self.test_storage)).exists:
                self.imaging_api.delete_folder(
                    requests.DeleteFolderRequest(folder, self.test_storage, True))

            self.assertFalse(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(folder, self.test_storage)).exists)

            self.imaging_api.copy_file(
                requests.CopyFileRequest(self.original_data_folder + '/' + file, folder + '/' + file,
                                         self.test_storage, self.test_storage))
            exists_response = self.imaging_api.object_exists(
                requests.ObjectExistsRequest(folder + '/' + file, self.test_storage))
            self.assertIsNotNone(exists_response)
            self.assertFalse(exists_response.is_folder)
            self.assertTrue(exists_response.exists)

            original_file_info = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
                self.original_data_folder, self.test_storage)).value if x.name == file)

            copied_file_info = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
            ...
            original_file = self.imaging_api.download_file(
                requests.DownloadFileRequest(self.original_data_folder + '/' + file, self.test_storage))
            copied_file = self.imaging_api.download_file(
                requests.DownloadFileRequest(folder + '/' + file, self.test_storage))
            ...
            self.imaging_api.delete_file(requests.DeleteFileRequest(folder + '/' + file, self.test_storage))
            self.assertFalse(
                self.imaging_api.object_exists(
                    requests.ObjectExistsRequest(folder + '/' + file, self.test_storage)).exists)
    ...
    def test_file_versions_copy(self):
        ...
        try:
            ...
            self.imaging_api.copy_file(
                requests.CopyFileRequest(self.original_data_folder + '/' + file1, folder + '/' + file1,
                                         self.test_storage, self.test_storage))
            self.imaging_api.copy_file(
                requests.CopyFileRequest(self.original_data_folder + '/' + file2, folder + '/' + file1,
                                         self.test_storage, self.test_storage))
            versions = self.imaging_api.get_file_versions(
                requests.GetFileVersionsRequest(folder + '/' + file1, self.test_storage)).value
            ...
            self.imaging_api.copy_file(
                requests.CopyFileRequest(folder + '/' + file1, folder + '/' + file1 + '.recent', self.test_storage,
                                         self.test_storage, recent_version.version_id))
            copied_versions = self.imaging_api.get_file_versions(
                requests.GetFileVersionsRequest(folder + '/' + file1 + '.recent', self.test_storage)).value
            ...
            self.imaging_api.copy_file(
                requests.CopyFileRequest(folder + '/' + file1, folder + '/' + file1 + '.old', self.test_storage,
                                         self.test_storage, old_version.version_id))
            copied_versions = self.imaging_api.get_file_versions(
                requests.GetFileVersionsRequest(folder + '/' + file1 + '.old', self.test_storage)).value
    ...
    def test_file_version_create(self):
        ...
        file1_size = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
            self.original_data_folder, self.test_storage)).value if x.name == file1).size

        file2_size = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
            self.original_data_folder + '/Folder1', self.test_storage)).value if
    ...
    def test_file_version_delete(self):
        ...
        try:
            ...
            self.assertTrue(
                self.imaging_api.object_exists(requests.ObjectExistsRequest(folder + '/' + file1, self.test_storage,
            ...
            self.imaging_api.delete_file(
                requests.DeleteFileRequest(folder + '/' + file1, self.test_storage, recent_version.version_id))
            ...
            self.imaging_api.delete_file(
                requests.DeleteFileRequest(folder + '/' + file1, self.test_storage, old_version.version_id))
            self.assertFalse(self.imaging_api.object_exists(requests.ObjectExistsRequest(folder + '/' + file1,
    ...
    def test_file_version_move(self):
        ...
        try:
            ...
            self.imaging_api.move_file(
                requests.MoveFileRequest(folder + '/' + file1, folder + '/' + file1 + '.recent',
                                         self.test_storage, self.test_storage, recent_version.version_id))
            copied_versions = self.imaging_api.get_file_versions(
                requests.GetFileVersionsRequest(folder + '/' + file1 + '.recent',
            ...
            self.imaging_api.move_file(requests.MoveFileRequest(folder + '/' + file1, folder + '/' + file1 + '.old',
                                                                self.test_storage, self.test_storage,
    ...
    def test_move_file(self):
        ...
        try:
            ...
            self.imaging_api.copy_file(
                requests.CopyFileRequest(self.original_data_folder + '/' + file, folder + '/' + file + '.copied',
                                         self.test_storage, self.test_storage))
            exist_response = self.imaging_api.object_exists(
                requests.ObjectExistsRequest(folder + '/' + file + '.copied', self.test_storage))
            self.assertIsNotNone(exist_response)
            self.assertFalse(exist_response.is_folder)
            self.assertTrue(exist_response.exists)

            self.imaging_api.move_file(requests.MoveFileRequest(folder + '/' + file + '.copied', folder + '/' + file,
                                                                self.test_storage, self.test_storage))
            self.assertFalse(
                self.imaging_api.object_exists(
                    requests.ObjectExistsRequest(folder + '/' + file + '.copied', self.test_storage)).exists)
            exist_response = self.imaging_api.object_exists(requests.ObjectExistsRequest(folder + '/' + file,
                                                                                         self.test_storage))
            self.assertIsNotNone(exist_response)
            self.assertFalse(exist_response.is_folder)
            self.assertTrue(exist_response.exists)
            ...
            original_file_info = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
                self.original_data_folder, self.test_storage)).value if x.name == file)
            moved_file_info = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
                folder, self.test_storage)).value if x.name == file)
    ...
    def test_upload_file(self):
        ...
        try:
            ...
            result = self.imaging_api.upload_file(
                requests.UploadFileRequest(folder + '/' + file, original_file, self.test_storage))
            ...
            uploaded_file_info = next(x for x in self.imaging_api.get_files_list(requests.GetFilesListRequest(
                folder, self.test_storage)).value if x.name == file)
